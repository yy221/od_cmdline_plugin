...
extern void set_load_hwnd ( HWND h);
...
extc int _export cdecl ODBG_Plugininit(int ollydbgversion,HWND hw,ulong *features) {
  ...
  sprintf(szIni,"%s\\%s",szPluginPath,"CmdBar.ini");
  ...
  sprintf(szMacDefFile,"%s\\%s",szPluginPath,"macro.def");
...
extc void _export cdecl ODBG_Pluginaction(int origin,int action,void *item) {
  ...
  case 1:
    ...
    if(ChooseFont(&cf)) {
      ...
      sprintf(s,"%d",lf.lfHeight);
      WritePrivateProfileString("Font","Size",s,szIni);
      sprintf(s,"%d",lf.lfCharSet);
