...
int Express(char *answer,ulong parm) {
  ...
  if(IsHex(v)) {
    ...
    val = strtol(v,&p,16);
    sprintf(answer, "HEX: %X - DEC: %d - ASCII: %s", val, val, tmp);
  }
  else {
    strcpy(answer,v);
  }
...
int Assembl(char *answer,ulong parm) {
  ...
  if (string[0]=='\0')                 // No immediate command
    ...
    if (pmem==NULL) {
      ...
      sprintf(answer,"%X",model.code[0]);
      for(i=1; i<model.length; i++) {
        sprintf(answer,"%s%X",answer,model.code[i]);
      }
      return -1; };
...
int LineAsm(char *answer,ulong parm) {
  ...
  // backup has the same base and size as the dump, so check it to avoid
  // strange ireproducible errors.
  sprintf(answer,"%02X",model.code[0]);
  for(i=1; i<model.length; i++) {
    sprintf(answer,"%s%02X",answer,model.code[i]);
  }
  return -1;
};
...
int LineDsm(char *answer,ulong parm) {
  ...
  if(n != len) {
    sprintf(answer,"%s \(%d/%d bytes disassemble\)",dsm.result,n,len);
  }
  else {
...
int Brkname(char *answer,ulong parm) {
  ...
  for(i=0; i<reftable->data.n; i++) {
    ...
    if(_stricmp(name,string)!=0) {      // Different function
      continue;
    }
...
int Heeeelp(char *answer,ulong parm) {
  ...
  if (n==0) {
    WinHelp((HWND)Plugingetvalue(VAL_HWMAIN),
     "cmdline.hlp",HELP_CONTENTS,0);
    return 0; };
  if (_stricmp(string,"OllyDbg")==0) {
    WinHelp((HWND)Plugingetvalue(VAL_HWMAIN),
...
static t_command cmdlist[] = {
  ...
  { "LOADPLUGIN", "S" , 0,            Loadplugin,"load plugin" },
  { "LOADPDB", "S" , 0,               Loadpdb,  "load pdb file"}, 
...
void SetCmdInfo(HWND hw, char *cmd)
{
  ...
  if(pdest) {
    *pdest = '\0';
    ...
    iHit = 0;
    for(i=0; i<sizeof(cmdlist)/sizeof(cmdlist[0]); i++) {
      if(!_strnicmp(cmd,cmdlist[i].cmd,strlen(cmd))) {
        cmdaddr = (DWORD*)realloc(cmdaddr,sizeof(DWORD)*(iHit+1));
        cmdaddr[iHit]  = (DWORD)&cmdlist[i];
        iHit++;
      }
  ...
  if(iHit > 1) {
    ...
    sprintf(cmdinfo," %s%s -- %s",hitcmd->cmd,buf,hitcmd->info);
    SetWindowText(hw,cmdinfo);
  }
  else {
    if(strlen(cmd) > 0) {
      sprintf(cmdinfo,"No valid command begin with \" %s \".",cmd);
      SetWindowText(hw,cmdinfo);
    }
    else {
      SetWindowText(hw,"");
...
int LoadMac(char *answer,ulong parm) {
  ...
  if(IsMacName == FALSE) {
    sprintf(answer,"\"%s\" is invalid macro name!!",string);
    return(-1);
  }
  ...
  do {
    ...
    if(strcmp(line,nomore)) {
      if((pnew = (PCOMMAND_QUE)calloc(1,sizeof(COMMAND_QUE))) == NULL) {
        sprintf(answer,"can't load Macro \"%s\"",string);
        FreeMacro();
        return -1;
      }
      // trim args
      ...
      for(j=0; j<sizeof(cmdlist)/sizeof(cmdlist[0]); j++) {
        //if(!_stricmp(buf,(pcmdlist+j)->cmd)) {
        if(!_stricmp(buf,cmdlist[j].cmd)) {
          sprintf(pnew->cmd,"%s",line);
          pnew->n = i;
          break;
        }
      }
